...
public class Obstacle {
    ...
    public java.awt.Shape projectedObstacle = AffineTransform.getScaleInstance(
        ...
        (1 + Constants.ROBOT_RADIUS * 2 / boundingBox.getWidth()))
            .createTransformedShape(obstacle); 
            // Scaling the obstacle to this factor. Increases all sides by the robot radius.

    /**
     * Obstacle constructor. If you want to make it easier for yourself, just use one of the
    ...
    public static Obstacle polygonObstacle(List<Translation2d> points) {
        ...
        while (iterator.hasNext()) {
            xPoints[iterator.nextIndex()] = 
                (int) points.get(iterator.nextIndex())
                    .getX();
            yPoints[iterator.nextIndex()] = 
                (int) points.get(iterator.nextIndex())
                    .getY();
     ...
     * @param position A Translation2d representing the position in 2D space of the CENTER of the ellipse.
    ...
    public static Obstacle ellipticalObstacle(Pose2d position, double height, double width) {
        return new Obstacle(AffineTransform.getRotateInstance(
     ...
     * @param position A Translation2d representing the position in 2D space of the CENTER of the rectangle.
