...
public class Path {
    ...
    public static boolean pointSeeable(Pose2d startPoint, Pose2d setPoint, List<Obstacle> obstaclelist) {
    ...
    public static boolean shapeIntersectsLine(java.awt.Shape shape, Line2D line) {
        ...
        while (!iterator.isDone()) {
            ...
            if (segmentType != PathIterator.SEG_MOVETO) {
                Line2D.Double segment = new Line2D.Double(prevCoords[0], prevCoords[1], currCoords[0], currCoords[1]);
    ...
    /** Nonstatic method to find out whether the goal of the robot can be driven straight to from where the robot is.
